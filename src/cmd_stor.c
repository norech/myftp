...
static void store_file(client_t *client, char *file, unsigned char *buffer)
{
    ...
    for (int nb_read; true; ) {
        nb_read = read(client->data_socket, buffer, 2048);
        if (nb_read == -1) {
            close(fd);
            return (void)send_ctrl_reply(client, ERR_ACTION_ABORTED_INTERNAL);
        }
        if (write(fd, buffer, nb_read) == -1) {
            close(fd);
...
void cmd_stor(client_t *client, int ac UNUSED, char *av[] UNUSED)
{
    char dir[PATH_MAX] = {0};
